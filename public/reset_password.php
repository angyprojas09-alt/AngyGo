...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['email_form'])) {
        // Formulario 1: Solicitar correo y generar token
        ...
        if (empty($correo)) {
            ...
            if ($stmt) {
                ...
                if ($stmt->num_rows > 0) {
                    ...
                    $stmt->bind_result($user_id, $nombre_usuario);
                    $stmt->fetch();
                    $stmt->close();
        ...
        if (empty($token) || empty($password_new) || empty($password_confirm)) {
            ...
            $check = $conexion->prepare("SELECT user_id FROM password_resets WHERE token = ? AND expires_at > NOW() AND used = 0 LIMIT 1");
